...
`;
  ...
  const [stage, setStage] = useState('IDLE'); // IDLE, READING_COMMITS, ANALYZING_PATTERNS, GENERATING_PATH, COMPLETE
...
${codeSnippet}`;
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-950 text-slate-50' : 'bg-gray-50 text-gray-900'}`}>
      {/* Navbar */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${isDarkMode ? 'border-white/5 bg-gray-950/80' : 'border-gray-200 bg-white/80'} border-b backdrop-blur-md`}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className={`hidden md:flex items-center space-x-8 text-sm font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            <a href="#features" className={`transition-colors ${isDarkMode ? 'hover:text-white' : 'hover:text-gray-900'}`}>Features</a>
            <a href="#how-it-works" className={`transition-colors ${isDarkMode ? 'hover:text-white' : 'hover:text-gray-900'}`}>How it works</a>
            <button 
              onClick={toggleTheme}
            ...
            <button 
              ...
              className={`transition-colors ${isDarkMode ? 'text-white hover:text-gray-300' : 'text-gray-900 hover:text-gray-600'}`}
            >
              Log in
            </button>
            <button 
              ...
              className={`px-4 py-2 rounded-full transition-colors ${isDarkMode ? 'bg-white text-black hover:bg-gray-200' : 'bg-gray-900 text-white hover:bg-gray-800'}`}
            >
              Get Started
            </button>
          </div>
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className={`md:hidden absolute top-16 left-0 right-0 border-b p-4 flex flex-col space-y-4 ${isDarkMode ? 'bg-gray-900 border-white/5' : 'bg-white border-gray-200'}`}>
            <a href="#features" className={`${isDarkMode ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-gray-900'}`}>Features</a>
            <a href="#how-it-works" className={`${isDarkMode ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-gray-900'}`}>How it works</a>
            <button 
              onClick={() => {
                setIsLoginMode(true);
                scrollToAuth();
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden">
        {/* Background Elements */}
        <div className={`absolute inset-0 ${isDarkMode ? 'bg-gray-950' : 'bg-gray-50'}`}>
        ...
        <div className="relative max-w-5xl mx-auto px-6 text-center z-10">
          <div className={`inline-flex items-center space-x-2 px-3 py-1 rounded-full border mb-8 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'}`}>
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
            </span>
            <span className={`text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Origin AI 2.0 is now live</span>
          </div>

          <h1 className={`text-5xl md:text-7xl font-bold tracking-tight mb-8 leading-tight ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
            Hiring based on <br />
          ...
          {/* Stats / Social Proof */}
          <div className={`mt-20 pt-10 border-t flex flex-wrap justify-center gap-10 md:gap-20 opacity-50 ${isDarkMode ? 'border-white/5' : 'border-gray-200'}`}>
            <div className="text-center">
              <div className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>10M+</div>
              <div className={`text-xs uppercase tracking-widest ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Lines Analyzed</div>
            </div>
            <div className="text-center">
              <div className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>85%</div>
              <div className={`text-xs uppercase tracking-widest ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Placement Rate</div>
            </div>
            <div className="text-center">
              <div className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>24/7</div>
              <div className={`text-xs uppercase tracking-widest ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>AI Agent Mentorship</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Features Grid */}
      <section id="features" className={`py-24 border-t transition-colors ${isDarkMode ? 'bg-gray-950 border-white/5' : 'bg-white border-gray-200'}`}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-16">
            <h2 className={`text-3xl md:text-4xl font-bold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>The Future of Work</h2>
            <p className={`max-w-2xl ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {[
              ...
              <div 
                key={idx}
                className={`group relative backdrop-blur-sm border rounded-2xl p-8 transition-colors duration-300 ${feature.span} ${isDarkMode ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}
              >
      ...
      {/* How It Works */}
      <section id="how-it-works" className={`py-24 transition-colors ${isDarkMode ? 'bg-gray-950' : 'bg-gray-50'}`}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className={`text-3xl md:text-4xl font-bold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>How It Works</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { num: 1, title: "Create Profile", desc: "Set up your profile with skills, interests, and availability to help our AI match you better.", color: "blue" },
              { num: 2, title: "Discover Projects", desc: "Browse through projects or use our smart search to find opportunities that match your interests.", color: "purple" },
              { num: 3, title: "Start Collaborating", desc: "Connect with project owners, chat about details, and start building something amazing together.", color: "green" }
            ].map((step, idx) => (
              <div key={idx} className="text-center">
                <div className={`w-16 h-16 bg-${step.color}-600 text-white rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-4`}>
                  {step.num}
                </div>
      ...
      {/* Interactive Demo - keeping mostly dark for code editor aesthetic */}
      <section id="demo" className={`py-24 relative overflow-hidden transition-colors ${isDarkMode ? 'bg-gray-950' : 'bg-gray-900'}`}>
        <div className={`absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] pointer-events-none ${isDarkMode ? 'from-blue-900/20 via-gray-950 to-gray-950' : 'from-blue-900/30 via-gray-900 to-gray-900'}`} />
        
        <div className="max-w-7xl mx-auto px-6 relative">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-white">See Origin in Action</h2>
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:h-[600px]">
            {/* Input Side */}
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-1 flex flex-col h-full overflow-hidden shadow-2xl shadow-blue-900/20">
              <div className="bg-black/50 px-4 py-3 flex items-center justify-between border-b border-white/10">
                <div className="flex items-center space-x-2">
                  <Terminal className="w-4 h-4 text-gray-400" />
                  <span className="text-sm text-gray-400 font-mono">commit_diff.patch</span>
                </div>
                <button 
              ...
              <textarea
                ...
                className="flex-1 bg-black/80 text-gray-300 font-mono text-sm p-4 resize-none focus:outline-none focus:ring-1 focus:ring-white/20"
                spellCheck={false}
              />
            ...
            <div className="flex flex-col h-full">
              {stage === 'IDLE' ? (
                <div className="h-full border border-white/10 rounded-xl border-dashed flex items-center justify-center text-gray-400">
                  <p>Run analysis to see skills extraction</p>
                </div>
              ) : (
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6 h-full overflow-y-auto transition-all duration-500">
                  <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center space-x-3">
                      <div className={`w-2 h-2 rounded-full ${stage === 'COMPLETE' ? 'bg-green-500' : 'bg-blue-500 animate-pulse'}`} />
                      <span className="text-sm font-medium tracking-wide uppercase text-gray-400">
                        {stage === 'COMPLETE' ? 'Analysis Complete' : stage.replace('_', ' ')}
                      </span>
                    </div>
                    {analysis && (
                      <div className="text-2xl font-bold text-white">
                        {analysis.complexityScore}<span className="text-sm text-gray-400 font-normal">/100 Impact</span>
                      </div>
                    )}
                  </div>
                  ...
                  {analysis && (
                    <div className="space-y-8">
                      {/* Skills Section */}
                      <div>
                        <h3 className="text-sm font-semibold text-gray-400 mb-3 uppercase tracking-wider">Detected Skills</h3>
                          ...
                          {analysis.suggestedCourses.map((course, i) => (
                            <div key={i} className="group cursor-pointer">
                              <div className="flex items-center justify-between mb-1">
                                <span className="text-sm font-medium text-white group-hover:text-emerald-400 transition-colors">{course.title}</span>
                                <span className="text-[10px] uppercase bg-white/10 px-2 py-0.5 rounded text-gray-400">{course.platform}</span>
                              </div>
                              <p className="text-xs text-gray-400 mb-2">{course.reason}</p>
                              <div className="w-full h-px bg-white/10 group-last:hidden" />
                            </div>
                          ))}
                        </div>
                        <button className="w-full mt-4 py-2 text-xs font-medium text-emerald-400 hover:text-emerald-300 flex items-center justify-center gap-1 transition-colors">
      ...
      {/* Auth Section */}
      <section id="auth-section" className={`py-24 relative overflow-hidden transition-colors ${isDarkMode ? 'bg-gray-950' : 'bg-gray-50'}`}>
        <div className={`absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] pointer-events-none ${isDarkMode ? 'from-blue-900/20 via-gray-950 to-gray-950' : 'from-blue-500/10 via-gray-50 to-gray-50'}`} />
        
        <div className="max-w-md mx-auto px-6 relative">
          <div className={`backdrop-blur-sm border rounded-2xl p-8 shadow-2xl ${isDarkMode ? 'bg-white/5 border-white/10 shadow-blue-900/20' : 'bg-white border-gray-200 shadow-gray-900/10'}`}>
      ...
      {/* CTA Section */}
      <section className={`py-24 relative overflow-hidden transition-colors ${isDarkMode ? 'bg-gray-950' : 'bg-white'}`}>
        <div className={`absolute inset-0 bg-gradient-to-b pointer-events-none ${isDarkMode ? 'from-gray-950 to-blue-900/10' : 'from-white to-blue-500/5'}`} />
      ...
      {/* Footer */}
      <footer className={`py-12 border-t transition-colors ${isDarkMode ? 'border-white/5 bg-gray-950' : 'border-gray-200 bg-white'}`}>
